    ...
    <div style={{ position:"absolute",right:0,top:"24px",zIndex:50,background:"#f8f7f4",border:"2px solid #1a1a1a",borderRadius:"4px",minWidth:"180px",boxShadow:"4px 4px 0 #1a1a1a" }}
      ...
      {isOwner ? (
        <>
          ...
          {post.forSale&&(
            <button onClick={async()=>{ await updateDoc(doc(db,"posts",post.id),{forSale:false,listPrice:null}); onClose(); }} style={{ display:"block",width:"100%",padding:"12px 16px",background:"none",border:"none",borderTop:"1px solid #e0ddd8",cursor:"pointer",fontSize:"13px",fontFamily:"inherit",textAlign:"left",color:"#888" }}>
              Remove Listing
            </button>
          )}
        </>
        ...
        <div style={{ padding:"0 24px 24px",maxHeight:"60vh",overflowY:"auto" }}>
            ...
            userComments.length===0
              ...
              : userComments.map((c,i)=>(
                <div key={i} style={{ padding:"14px 0",borderBottom:"1px solid #e0ddd8" }}>
                  <div style={{ fontSize:"12px",color:"#888",marginBottom:"4px" }}>Replying to {c.postHandle} · {c.timestamp?timeAgo({toDate:()=>new Date(c.timestamp)}):"Just now"}</div>
          ...
          {activeTab==="holdings" && showHoldings && (
            userHoldings.length===0
              ...
              : userHoldings.map(p=>(
                <div key={p.id} style={{ padding:"16px 0",borderBottom:"1px solid #e0ddd8" }}>
                  <div style={{ fontSize:"12px",color:"#888",marginBottom:"4px" }}>Originally by {p.handle}</div>
                  <MiniPost p={p}/>
                </div>
              ))
          )}
          {activeTab==="likes" && showLikes && (
            userLikes.length===0
              ...
              : userLikes.map(p=>(
                <div key={p.id} style={{ padding:"16px 0",borderBottom:"1px solid #e0ddd8" }}>
                  <div style={{ fontSize:"12px",color:"#888",marginBottom:"4px",display:"flex",alignItems:"center",gap:"6px" }}>
                    <span style={{ cursor:"pointer",fontWeight:"700",color:"#1a1a1a" }} onClick={()=>onOpenProfile(p.userId)}>{p.handle}</span>
                  </div>
                  <MiniPost p={p}/>
                </div>
              ))
              ...
              : userOffers.map(o=>(
                <div key={o.id} style={{ padding:"14px 0",borderBottom:"1px solid #e0ddd8" }}>
                  <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"4px" }}>
                    ...
                    <span style={{ fontSize:"11px",fontWeight:"700",padding:"2px 8px",borderRadius:"2px",background:o.status==="completed"?"#d1fae5":o.status==="accepted"?"#bfdbfe":o.status==="declined"?"#fecaca":"#fde68a",color:o.status==="completed"?"#065f46":o.status==="accepted"?"#1e40af":o.status==="declined"?"#991b1b":"#92400e" }}>
        ...
        <div style={{ borderTop:"1px solid #e0ddd8",paddingTop:"16px",marginBottom:"20px" }}>
          ...
          {[
            ...
            <div key={label} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px" }}>
              ...
              <button onClick={()=>set(!value)} style={{ width:"44px",height:"24px",borderRadius:"12px",border:"2px solid #1a1a1a",background:value?"#1a1a1a":"#e0ddd8",cursor:"pointer",position:"relative",transition:"background 0.2s",flexShrink:0 }}>
                <span style={{ position:"absolute",top:"2px",left:value?"20px":"2px",width:"16px",height:"16px",borderRadius:"50%",background:"#f8f7f4",transition:"left 0.2s" }}/>
              </button>
            </div>
          ))}
        </div>
...
export default function NformApp() {
  ...
  useEffect(() => {
    const handler = () => { setOpenMenuPostId(null); setShowProfileMenu(false); setShowNavMenu(false); };
  ...
  const myOffers        = offers.filter(o=>o.status==="pending"&&(o.currentOwnerId===user?.uid||(!o.currentOwnerId&&o.originalPosterId===user?.uid)));
  ...
  async function submitOffer() {
    ...
    if (offerModal.post.listPrice&&Number(offerAmount)>Number(offerModal.post.listPrice)) { setOfferError(`Cannot exceed listing price of $${offerModal.post.listPrice}.`); return; }
    setShowPayPal(true);
  }

  async function onPayPalApprove(post, amount) {
    await updateDoc(doc(db,"posts",post.id), { currentOwnerId:user.uid, currentOwnerHandle:"@"+(displayName||user.displayName).toLowerCase().replace(/\s/g,""), forSale:false, listPrice:null });
    await addDoc(collection(db,"offers"), { postId:post.id, postContent:post.content, originalPosterId:post.userId, originalPosterHandle:post.handle, currentOwnerId:post.currentOwnerId||post.userId, buyerId:user.uid, buyerHandle:"@"+(displayName||user.displayName).toLowerCase().replace(/\s/g,""), amount:Number(amount), status:"completed", createdAt:serverTimestamp() });
    setOfferModal(null); setShowPayPal(false); setOfferAmount("");
    alert("🎉 You now own this post! Find it in your Holdings.");
  }

  async function acceptOffer(offer) {
    await updateDoc(doc(db,"posts",offer.postId), { currentOwnerId:offer.buyerId, currentOwnerHandle:offer.buyerHandle, forSale:false, listPrice:null });
    await updateDoc(doc(db,"offers",offer.id), { status:"accepted" });
  }
  async function declineOffer(offerId) { await updateDoc(doc(db,"offers",offerId),{status:"declined"}); }
    ...
    <div style={{ fontFamily:"'Georgia','Times New Roman',serif",background:"#f8f7f4",minHeight:"100vh",color:"#1a1a1a" }}>
      ...
      {offerModal && (
        <div style={{ position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px" }}
          ...
          <div style={{ background:"#f8f7f4",border:"2px solid #1a1a1a",borderRadius:"4px",width:"100%",maxWidth:"440px",padding:"28px",boxShadow:"6px 6px 0 #1a1a1a" }}>
            <h2 style={{ margin:"0 0 6px",fontSize:"18px",fontWeight:"900",fontStyle:"italic" }}>Make an Offer</h2>
            ...
            {offerModal.post.listPrice&&<div style={{ marginBottom:"12px",fontSize:"13px",color:"#666" }}>Listed at: <strong style={{ color:"#1a1a1a" }}>${offerModal.post.listPrice}</strong></div>}
            {!showPayPal ? (
              <>
                ...
                <div style={{ display:"flex",gap:"10px",marginTop:"8px" }}>
                  ...
                  <button onClick={submitOffer} style={{ flex:1,padding:"10px",background:"#1a1a1a",color:"#f8f7f4",border:"2px solid #1a1a1a",cursor:"pointer",fontSize:"13px",fontWeight:"700",fontFamily:"inherit" }}>Continue →</button>
                </div>
              </>
            ):(
              <>
                <div style={{ marginBottom:"12px",fontSize:"14px",fontWeight:"600" }}>Pay <span style={{ color:"#1a1a1a" }}>${Number(offerAmount).toFixed(2)}</span> via PayPal:</div>
                <PayPalButtons style={{ layout:"vertical",shape:"rect",label:"pay" }}
                  ...
                  onError={()=>setOfferError("Payment failed.")}/>
        ...
        <div style={{ position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px" }}
          ...
          <div style={{ background:"#f8f7f4",border:"2px solid #1a1a1a",borderRadius:"4px",width:"100%",maxWidth:"400px",padding:"28px",boxShadow:"6px 6px 0 #1a1a1a" }}>
            ...
            <input type="number" min="0.01" step="0.01" value={listPrice} onChange={e=>setListPrice(e.target.value)} placeholder="e.g. 50.00"
              style={{ width:"100%",padding:"10px 14px",border:"2px solid #1a1a1a",borderRadius:"2px",fontSize:"16px",fontFamily:"inherit",outline:"none",boxSizing:"border-box",marginBottom:"8px" }}/>
            <p style={{ fontSize:"12px",color:"#888",margin:"0 0 16px" }}>Buyers cannot offer above this price.</p>
      ...
      <header style={{ position:"sticky",top:0,zIndex:100,background:"rgba(248,247,244,0.97)",backdropFilter:"blur(12px)",borderBottom:"2px solid #1a1a1a",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:"60px",gap:"12px" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:"8px",flexShrink:0 }}>
          {user ? (
            <>
              {/* Vertical three-dot nav menu */}
              <div style={{ position:"relative" }}>
                <button onClick={e=>{ e.stopPropagation(); setShowNavMenu(!showNavMenu); setShowProfileMenu(false); }}
                  style={{ background:"none",border:"2px solid #1a1a1a",borderRadius:"2px",padding:"5px 10px",cursor:"pointer",fontSize:"16px",fontWeight:"900",lineHeight:1,color:"#1a1a1a",letterSpacing:"1px" }}>
                  ⋮
                </button>
                {showNavMenu && (
                  <div onClick={e=>e.stopPropagation()} style={{ position:"absolute",right:0,top:"40px",zIndex:200,background:"#f8f7f4",border:"2px solid #1a1a1a",borderRadius:"4px",minWidth:"180px",boxShadow:"4px 4px 0 #1a1a1a" }}>
                    {[
                      ...
                      <button key={item.feed} onClick={()=>{ setActiveFeed(item.feed); setShowNavMenu(false); }}
                        ...
                        {item.label}
                        {item.feed==="holdings"&&myOffers.length>0&&<span style={{ marginLeft:"6px",background:"#dc2626",color:"#fff",borderRadius:"10px",padding:"1px 6px",fontSize:"11px" }}>{myOffers.length}</span>}
                      </button>
                    ))}
                  </div>
                )}
      ...
      <div style={{ display:"flex",maxWidth:"1200px",margin:"0 auto" }}>
        ...
        <aside style={{ width:sidebarOpen?"240px":"0",overflow:"hidden",transition:"width 0.3s ease",borderRight:sidebarOpen?"2px solid #1a1a1a":"none",background:"#f8f7f4",position:"sticky",top:"60px",height:"calc(100vh - 60px)",flexShrink:0 }}>
          ...
          {user&&<>
            {[{label:"◆ Bookmarks",feed:"bookmarks"},{label:"❤️ Likes",feed:"likes"},{label:"🏦 Holdings",feed:"holdings"}].map(item=>(
              <button key={item.feed} onClick={()=>{ setActiveFeed(item.feed); setSidebarOpen(false); }}
                style={{ display:"flex",alignItems:"center",gap:"10px",width:"100%",padding:"10px 16px",background:activeFeed===item.feed?"#1a1a1a":"none",color:activeFeed===item.feed?"#f8f7f4":"#1a1a1a",border:"none",borderTop:item.feed==="bookmarks"?"2px solid #e0ddd8":"none",cursor:"pointer",fontSize:"14px",textAlign:"left",fontFamily:"inherit",marginTop:item.feed==="bookmarks"?"8px":"0" }}>
                <span>{item.label}</span>
                {item.feed==="holdings"&&myOffers.length>0&&<span style={{ background:"#dc2626",color:"#fff",borderRadius:"10px",padding:"1px 6px",fontSize:"11px" }}>{myOffers.length}</span>}
              </button>
            ))}
          </>}
        </aside>
        ...
        <main style={{ flex:1 }}>
          ...
          {/* Section headers for non-home feeds */}
          {activeFeed!=="home"&&(
            <div style={{ padding:"20px 24px",borderBottom:"2px solid #1a1a1a",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
              <div>
                ...
                <p style={{ margin:"4px 0 0",color:"#888",fontSize:"13px" }}>
                  {activeFeed==="bookmarks"?`${bookmarks.length} saved`:activeFeed==="likes"?`${likedPosts.length} liked`:`${holdingsPosts.length} owned`}
                </p>
              </div>
          ...
          {/* Incoming offers panel inside Holdings */}
          {activeFeed==="holdings"&&myOffers.length>0&&(
          ...
          {activeFeed==="home"&&(
            <div style={{ borderBottom:"2px solid #1a1a1a",padding:"16px 24px",background:"#fff" }}>
              {user ? (
                ...
                <div style={{ border:"2px solid #1a1a1a",borderRadius:"4px",overflow:"hidden",cursor:"pointer" }} onClick={()=>setSignInPrompt("Sign in to share your news with the world!")}>
                  <div style={{ padding:"16px 20px",background:"#fff",fontSize:"15px",color:"#aaa",fontFamily:"Georgia,serif",fontStyle:"italic" }}>Share your news with the world!</div>
                </div>
              )}
            </div>
          )}
          ...
          {loading ? (
            ...
            <div style={{ padding:"60px",textAlign:"center",color:"#888",fontSize:"16px" }}>
              ...
              :activeFeed==="holdings"?"You don't own any posts yet."
            ...
            <div style={{ border:"2px solid #1a1a1a",borderRadius:"4px",background:"#f8f7f4",overflow:"hidden" }}>
              ...
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",borderTop:"1px solid #e0ddd8",background:"#f0ede8" }}>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:"12px" }}>
                  ...
                  <button onClick={async()=>{ await handlePost(); setShowCompose(false); }} style={{ background:"#1a1a1a",color:"#f8f7f4",border:"none",borderRadius:"2px",padding:"8px 20px",fontSize:"13px",fontWeight:"700",cursor:"pointer",letterSpacing:"1px",fontFamily:"inherit" }}>Publish →</button>
                </div>
              </div>
            </div>
